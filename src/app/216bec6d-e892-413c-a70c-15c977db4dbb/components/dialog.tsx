...
function DialogContent({ children, className, defaultDepth, ...props }: { children?: React.ReactNode, className?: string, defaultDepth?: number } & React.ComponentProps<"div">) {
    return (
        <div className="left-[50%] max-w-[calc(100%-2rem)] sm:max-w-[425px] fixed top-[50%] translate-x-[-50%] translate-y-[-50%] w-full z-50">
            {(defaultDepth ?? 0) > 0 && (
            ...
            <div
                ...
                className={cn(
                    "bg-white border shadow-sm rounded-lg flex flex-col gap-4 max-h-[90vh] p-6 w-full z-50 relative",
                    className
                )}
                {...props}
            >
